                    ...
                    @if($report->count())
                        @foreach($report as $_report)
                            <tr>
                                ...
                                <td>{{ $_report->get('repayment_amount_due') }}</td>
                                <td>{{ $_report->get('loan_amount_collected') }}</td>
                                <td>{{ $_report->get('percentage_collected') }}</td>
                                <td>{{ $_report->get('credit_officer') }}</td>
                            </tr>
                        @endforeach
                        <tr style="font-weight: bolder">
                            <td colspan="3">&nbsp;</td>
                            <td>{{ number_format($report->totals->get('repayment_amount_due'), 2) }}</td>
                            <td>{{ number_format($report->totals->get('loan_amount_collected'), 2) }}</td>
                            <td>{{ number_format($report->totals->get('percentage_collected'), 2) }}</td>
                            <td>&nbsp;</td>
                        </tr>
                    @else
                        <tr>
